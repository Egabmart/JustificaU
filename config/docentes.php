<?php

// config/docentes.php
// Estructura de datos con los docentes y sus correos para cada grupo.

return [
    'Administración de Empresas' => [
        'Primer Año' => [
            'Fundamentos de Administración' => [
                '1' => ['nombre' => 'Lic. Carlos Mendoza', 'email' => 'cmendoza@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Sofia Vargas', 'email' => 'svargas@uam.edu.ni'],
            ],
            'Contabilidad I' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
            'Matemática I' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
            'Comunicación Oral y Escrita' => [
                '1' => ['nombre' => 'Lic. Lucia Cordero', 'email' => 'lcordero@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Fernando Nuñez', 'email' => 'fnunez@uam.edu.ni'],
            ],
            'Sociología' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
            'Microeconomía' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
            ],
            'Contabilidad II' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
            'Estadística I' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
        ],
        'Segundo Año' => [
            'Administración de RRHH' => [
                '1' => ['nombre' => 'Lic. Sofia Vargas', 'email' => 'svargas@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carlos Mendoza', 'email' => 'cmendoza@uam.edu.ni'],
            ],
            'Macroeconomía' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
            ],
            'Estadística II' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
            'Matemática Financiera' => [
                '1' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
            ],
            'Contabilidad de Costos' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
            'Derecho Mercantil' => [
                '1' => ['nombre' => 'Lic. Fernando Nuñez', 'email' => 'fnunez@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Lucia Cordero', 'email' => 'lcordero@uam.edu.ni'],
            ],
            'Mercadotecnia I' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
            'Presupuestos' => [
                '1' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
            ],
        ],
        'Tercer Año' => [
            'Administración Financiera I' => [
                '1' => ['nombre' => 'Lic. Ricardo Salas', 'email' => 'rsalas@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Monica Falla', 'email' => 'mfalla@uam.edu.ni'],
            ],
            'Mercadotecnia II' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
            'Administración de Operaciones I' => [
                '1' => ['nombre' => 'Lic. Alberto Pinto', 'email' => 'apinto@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diana Quiros', 'email' => 'dquiros@uam.edu.ni'],
            ],
            'Organización y Métodos' => [
                '1' => ['nombre' => 'Lic. Marcela Solis', 'email' => 'msolis@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Oscar Barquero', 'email' => 'obarquero@uam.edu.ni'],
            ],
            'Administración Financiera II' => [
                '1' => ['nombre' => 'Lic. Ricardo Salas', 'email' => 'rsalas@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Monica Falla', 'email' => 'mfalla@uam.edu.ni'],
            ],
            'Investigación de Mercados' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
            'Administración de Operaciones II' => [
                '1' => ['nombre' => 'Lic. Alberto Pinto', 'email' => 'apinto@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diana Quiros', 'email' => 'dquiros@uam.edu.ni'],
            ],
            'Formulación y Evaluación de Proyectos' => [
                '1' => ['nombre' => 'Lic. Alejandro Velez', 'email' => 'avelez@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carmen Blanco', 'email' => 'cblanco@uam.edu.ni'],
            ],
        ],
        'Cuarto Año' => [
            'Administración de Ventas' => [
                '1' => ['nombre' => 'Lic. Ismael Nieto', 'email' => 'inieto@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Patricia Montes', 'email' => 'pmontes@uam.edu.ni'],
            ],
            'Gerencia Estratégica' => [
                '1' => ['nombre' => 'Lic. Roberto Arias', 'email' => 'rarias@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carolina Soto', 'email' => 'csoto@uam.edu.ni'],
            ],
            'Comercio Electrónico' => [
                '1' => ['nombre' => 'Lic. Esteban Vega', 'email' => 'evega@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Laura Perez', 'email' => 'lperez@uam.edu.ni'],
            ],
            'Simulación de Negocios' => [
                '1' => ['nombre' => 'Lic. Carlos Mendoza', 'email' => 'cmendoza@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Sofia Vargas', 'email' => 'svargas@uam.edu.ni'],
            ],
            'Derecho Laboral' => [
                '1' => ['nombre' => 'Lic. Fernando Nuñez', 'email' => 'fnunez@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Lucia Cordero', 'email' => 'lcordero@uam.edu.ni'],
            ],
            'Logística y Distribución' => [
                '1' => ['nombre' => 'Lic. Alberto Pinto', 'email' => 'apinto@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diana Quiros', 'email' => 'dquiros@uam.edu.ni'],
            ],
            'Calidad Total' => [
                '1' => ['nombre' => 'Lic. Marcela Solis', 'email' => 'msolis@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Oscar Barquero', 'email' => 'obarquero@uam.edu.ni'],
            ],
            'Taller de Tesis' => [
                '1' => ['nombre' => 'Lic. Alejandro Velez', 'email' => 'avelez@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carmen Blanco', 'email' => 'cblanco@uam.edu.ni'],
            ],
        ],
        'Quinto Año' => [
            'Prácticas Profesionales' => [
                '1' => ['nombre' => 'Lic. Ricardo Salas', 'email' => 'rsalas@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Monica Falla', 'email' => 'mfalla@uam.edu.ni'],
            ],
            'Proyecto de Titulación' => [
                '1' => ['nombre' => 'Lic. Roberto Arias', 'email' => 'rarias@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carolina Soto', 'email' => 'csoto@uam.edu.ni'],
            ],
        ],
    ],
    'Contabilidad y Finanzas' => [
        'Primer Año' => [
            'Fundamentos de Contabilidad' => [
                '1' => ['nombre' => 'Lic. Roberto Arias', 'email' => 'rarias@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carolina Soto', 'email' => 'csoto@uam.edu.ni'],
            ],
            'Matemática I' => [
                '1' => ['nombre' => 'Lic. Esteban Vega', 'email' => 'evega@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Laura Perez', 'email' => 'lperez@uam.edu.ni'],
            ],
            'Administración I' => [
                '1' => ['nombre' => 'Lic. Carlos Mendoza', 'email' => 'cmendoza@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Sofia Vargas', 'email' => 'svargas@uam.edu.ni'],
            ],
            'Derecho Constitucional' => [
                '1' => ['nombre' => 'Lic. Fernando Nuñez', 'email' => 'fnunez@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Lucia Cordero', 'email' => 'lcordero@uam.edu.ni'],
            ],
            'Contabilidad II' => [
                '1' => ['nombre' => 'Lic. Roberto Arias', 'email' => 'rarias@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carolina Soto', 'email' => 'csoto@uam.edu.ni'],
            ],
            'Microeconomía' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
            ],
            'Estadística I' => [
                '1' => ['nombre' => 'Lic. Esteban Vega', 'email' => 'evega@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Laura Perez', 'email' => 'lperez@uam.edu.ni'],
            ],
            'Informática Aplicada' => [
                '1' => ['nombre' => 'Ing. Marcela Solis', 'email' => 'msolis@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Oscar Barquero', 'email' => 'obarquero@uam.edu.ni'],
            ],
        ],
        'Segundo Año' => [
            'Contabilidad de Costos I' => [
                '1' => ['nombre' => 'Lic. Roberto Arias', 'email' => 'rarias@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carolina Soto', 'email' => 'csoto@uam.edu.ni'],
            ],
            'Macroeconomía' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
            ],
            'Matemática Financiera' => [
                '1' => ['nombre' => 'Lic. Esteban Vega', 'email' => 'evega@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Laura Perez', 'email' => 'lperez@uam.edu.ni'],
            ],
            'Derecho Mercantil' => [
                '1' => ['nombre' => 'Lic. Fernando Nuñez', 'email' => 'fnunez@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Lucia Cordero', 'email' => 'lcordero@uam.edu.ni'],
            ],
            'Contabilidad de Sociedades' => [
                '1' => ['nombre' => 'Lic. Roberto Arias', 'email' => 'rarias@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carolina Soto', 'email' => 'csoto@uam.edu.ni'],
            ],
            'Finanzas I' => [
                '1' => ['nombre' => 'Lic. Ricardo Salas', 'email' => 'rsalas@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Monica Falla', 'email' => 'mfalla@uam.edu.ni'],
            ],
            'Tributación I' => [
                '1' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
            ],
            'Mercados Financieros' => [
                '1' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
            ],
        ],
        'Tercer Año' => [
            'Auditoría I' => [
                '1' => ['nombre' => 'Lic. Ismael Nieto', 'email' => 'inieto@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Patricia Montes', 'email' => 'pmontes@uam.edu.ni'],
            ],
            'Finanzas II' => [
                '1' => ['nombre' => 'Lic. Ricardo Salas', 'email' => 'rsalas@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Monica Falla', 'email' => 'mfalla@uam.edu.ni'],
            ],
            'Tributación II' => [
                '1' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
            ],
            'Contabilidad de Costos II' => [
                '1' => ['nombre' => 'Lic. Roberto Arias', 'email' => 'rarias@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carolina Soto', 'email' => 'csoto@uam.edu.ni'],
            ],
            'Auditoría II' => [
                '1' => ['nombre' => 'Lic. Ismael Nieto', 'email' => 'inieto@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Patricia Montes', 'email' => 'pmontes@uam.edu.ni'],
            ],
            'Análisis de Estados Financieros' => [
                '1' => ['nombre' => 'Lic. Ricardo Salas', 'email' => 'rsalas@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Monica Falla', 'email' => 'mfalla@uam.edu.ni'],
            ],
            'Formulación de Proyectos' => [
                '1' => ['nombre' => 'Lic. Alejandro Velez', 'email' => 'avelez@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carmen Blanco', 'email' => 'cblanco@uam.edu.ni'],
            ],
            'Normas Internacionales (NIIF)' => [
                '1' => ['nombre' => 'Lic. Roberto Arias', 'email' => 'rarias@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carolina Soto', 'email' => 'csoto@uam.edu.ni'],
            ],
        ],
        'Cuarto Año' => [
            'Auditoría III' => [
                '1' => ['nombre' => 'Lic. Ismael Nieto', 'email' => 'inieto@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Patricia Montes', 'email' => 'pmontes@uam.edu.ni'],
            ],
            'Finanzas Corporativas' => [
                '1' => ['nombre' => 'Lic. Ricardo Salas', 'email' => 'rsalas@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Monica Falla', 'email' => 'mfalla@uam.edu.ni'],
            ],
            'Contabilidad Gubernamental' => [
                '1' => ['nombre' => 'Lic. Roberto Arias', 'email' => 'rarias@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carolina Soto', 'email' => 'csoto@uam.edu.ni'],
            ],
            'Auditoría de Sistemas' => [
                '1' => ['nombre' => 'Ing. Marcela Solis', 'email' => 'msolis@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Oscar Barquero', 'email' => 'obarquero@uam.edu.ni'],
            ],
            'Contabilidad Avanzada' => [
                '1' => ['nombre' => 'Lic. Roberto Arias', 'email' => 'rarias@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carolina Soto', 'email' => 'csoto@uam.edu.ni'],
            ],
            'Consultoría Financiera' => [
                '1' => ['nombre' => 'Lic. Ricardo Salas', 'email' => 'rsalas@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Monica Falla', 'email' => 'mfalla@uam.edu.ni'],
            ],
            'Ética Profesional' => [
                '1' => ['nombre' => 'Lic. Lucia Cordero', 'email' => 'lcordero@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Fernando Nuñez', 'email' => 'fnunez@uam.edu.ni'],
            ],
            'Seminario de Tesis' => [
                '1' => ['nombre' => 'Lic. Alejandro Velez', 'email' => 'avelez@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carmen Blanco', 'email' => 'cblanco@uam.edu.ni'],
            ],
        ],
        'Quinto Año' => [
            'Prácticas Profesionales' => [
                '1' => ['nombre' => 'Lic. Sofia Vargas', 'email' => 'svargas@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carlos Mendoza', 'email' => 'cmendoza@uam.edu.ni'],
            ],
            'Examen de Grado' => [
                '1' => ['nombre' => 'Lic. Roberto Arias', 'email' => 'rarias@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Carolina Soto', 'email' => 'csoto@uam.edu.ni'],
            ],
        ],
    ],
    'Economía Empresarial' => [
        'Primer Año' => [
            'Principios de Economía' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
            ],
            'Matemática Básica' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
            'Contabilidad I' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
            'Introducción a las Ciencias Administrativas' => [
                '1' => ['nombre' => 'Lic. Carlos Mendoza', 'email' => 'cmendoza@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Sofia Vargas', 'email' => 'svargas@uam.edu.ni'],
            ],
            'Microeconomía I' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
            ],
            'Cálculo Aplicado I' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
            'Contabilidad II' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
            'Competencias para Emprender' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
        ],
        'Segundo Año' => [
            'Macroeconomía II' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'valerialeon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dgmora@uam.edu.ni'],
            ],
            'Cálculo Aplicado II' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
            'Estadística I' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
            'Macroeconomía I' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
            ],
            'Pensamiento Económico' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
            'Matemática Financiera' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
            'Estadística II' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
            'Macroeconomía II' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
            ],
        ],
        'Tercer Año' => [
            'Análisis de los Estados Financieros' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'valerialeon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dgmora@uam.edu.ni'],
            ],
            'Investigación de Operaciones' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
            'Finanzas I' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
            'Macroeconomía III' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
            ],
            'Mercadeo' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
            'Teoría de Juegos' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
            'Finanzas II' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
            'Econometría I' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
            ],
        ],
        'Cuarto Año' => [
            'Economía Internacional' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'valerialeon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dgmora@uam.edu.ni'],
            ],
            'Econometría II' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
            'Evaluación de Proyectos de Inversión' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
            'Historia Económica' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
            ],
            'Política Económica' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
            'Economía del Sector Público' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
            'Seminario de Tópicos Económicos' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
            'Taller de Investigación' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
            ],
        ],
        'Quinto Año' => [
            'Prácticas de Especialización' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'valerialeon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dgmora@uam.edu.ni'],
            ],
            'Tesis de Grado' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
        ],
    ],
    'Negocios Internacionales' => [
        'Primer Año' => [
            'Administración Global' => [
                '1' => ['nombre' => 'Lic. Carlos Mendoza', 'email' => 'cmendoza@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Sofia Vargas', 'email' => 'svargas@uam.edu.ni'],
            ],
            'Contabilidad Financiera' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
            'Economía Global' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
            ],
            'Comunicación Intercultural' => [
                '1' => ['nombre' => 'Lic. Lucia Cordero', 'email' => 'lcordero@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Fernando Nuñez', 'email' => 'fnunez@uam.edu.ni'],
            ],
            'Marketing Internacional' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
            'Derecho Internacional Público' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
            'Finanzas Globales' => [
                '1' => ['nombre' => 'Lic. Ricardo Salas', 'email' => 'rsalas@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Monica Falla', 'email' => 'mfalla@uam.edu.ni'],
            ],
            'Estadística para Negocios' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
        ],
        'Segundo Año' => [
            'Logística y Cadena de Suministro' => [
                '1' => ['nombre' => 'Lic. Carlos Mendoza', 'email' => 'cmendoza@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Sofia Vargas', 'email' => 'svargas@uam.edu.ni'],
            ],
            'Investigación de Mercados Globales' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
            'Tratados y Acuerdos Comerciales' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
            ],
            'Gestión de RRHH Global' => [
                '1' => ['nombre' => 'Lic. Lucia Cordero', 'email' => 'lcordero@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Fernando Nuñez', 'email' => 'fnunez@uam.edu.ni'],
            ],
            'Estrategia de Negocios Internacionales' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
            'Cultura y Ética en los Negocios' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
            'Operaciones de Importación y Exportación' => [
                '1' => ['nombre' => 'Lic. Ricardo Salas', 'email' => 'rsalas@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Monica Falla', 'email' => 'mfalla@uam.edu.ni'],
            ],
            'Análisis de Riesgo País' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
        ],
        'Tercer Año' => [
            'Geopolítica y Negocios' => [
                '1' => ['nombre' => 'Lic. Carlos Mendoza', 'email' => 'cmendoza@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Sofia Vargas', 'email' => 'svargas@uam.edu.ni'],
            ],
            'Marketing Digital Global' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
            'Contratos Internacionales' => [
                '1' => ['nombre' => 'Lic. Valeria Leon', 'email' => 'vleon@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Diego Mora', 'email' => 'dmora@uam.edu.ni'],
            ],
            'Simulación de Negocios Globales' => [
                '1' => ['nombre' => 'Lic. Lucia Cordero', 'email' => 'lcordero@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Fernando Nuñez', 'email' => 'fnunez@uam.edu.ni'],
            ],
            'Gestión de Proyectos Internacionales' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
            'Finanzas Corporativas Internacionales' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
            'Emprendimiento Global' => [
                '1' => ['nombre' => 'Lic. Ricardo Salas', 'email' => 'rsalas@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Monica Falla', 'email' => 'mfalla@uam.edu.ni'],
            ],
            'Taller de Graduación I' => [
                '1' => ['nombre' => 'Lic. Ana Guzman', 'email' => 'aguzman@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Jorge Castro', 'email' => 'jcastro@uam.edu.ni'],
            ],
        ],
        'Cuarto Año' => [
            'Prácticas Profesionales' => [
                '1' => ['nombre' => 'Lic. Carlos Mendoza', 'email' => 'cmendoza@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Sofia Vargas', 'email' => 'svargas@uam.edu.ni'],
            ],
            'Taller de Graduación II' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
        ],
    ],
    'Derecho' => [
        'Primer Año' => [
            'Introducción al Estudio del Derecho' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
            'Derecho Romano' => [
                '1' => ['nombre' => 'Lic. Aitor Tilla', 'email' => 'atilla@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elba Lazo', 'email' => 'elazo@uam.edu.ni'],
            ],
            'Teoría del Estado' => [
                '1' => ['nombre' => 'Lic. Aquiles Bailo', 'email' => 'abailo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Ines Quele', 'email' => 'iquele@uam.edu.ni'],
            ],
            'Sociología Jurídica' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
            'Derecho de Personas y Familia' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
            'Teoría General del Proceso' => [
                '1' => ['nombre' => 'Lic. Aitor Tilla', 'email' => 'atilla@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elba Lazo', 'email' => 'elazo@uam.edu.ni'],
            ],
            'Derecho Constitucional I' => [
                '1' => ['nombre' => 'Lic. Aquiles Bailo', 'email' => 'abailo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Ines Quele', 'email' => 'iquele@uam.edu.ni'],
            ],
        ],
        'Segundo Año' => [
            'Derecho de Bienes' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
            'Derecho de Obligaciones' => [
                '1' => ['nombre' => 'Lic. Aitor Tilla', 'email' => 'atilla@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elba Lazo', 'email' => 'elazo@uam.edu.ni'],
            ],
            'Derecho Penal I' => [
                '1' => ['nombre' => 'Lic. Aquiles Bailo', 'email' => 'abailo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Ines Quele', 'email' => 'iquele@uam.edu.ni'],
            ],
            'Derecho Constitucional II' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
            'Derecho de Sucesiones' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
            'Contratos Civiles' => [
                '1' => ['nombre' => 'Lic. Aitor Tilla', 'email' => 'atilla@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elba Lazo', 'email' => 'elazo@uam.edu.ni'],
            ],
            'Derecho Penal II' => [
                '1' => ['nombre' => 'Lic. Aquiles Bailo', 'email' => 'abailo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Ines Quele', 'email' => 'iquele@uam.edu.ni'],
            ],
            'Derecho Administrativo I' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
        ],
        'Tercer Año' => [
            'Derecho Procesal Civil I' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
            'Derecho Mercantil I' => [
                '1' => ['nombre' => 'Lic. Aitor Tilla', 'email' => 'atilla@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elba Lazo', 'email' => 'elazo@uam.edu.ni'],
            ],
            'Derecho Laboral' => [
                '1' => ['nombre' => 'Lic. Aquiles Bailo', 'email' => 'abailo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Ines Quele', 'email' => 'iquele@uam.edu.ni'],
            ],
            'Derecho Administrativo II' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
            'Derecho Procesal Civil II' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
            'Derecho Mercantil II' => [
                '1' => ['nombre' => 'Lic. Aitor Tilla', 'email' => 'atilla@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elba Lazo', 'email' => 'elazo@uam.edu.ni'],
            ],
            'Derecho Procesal Penal' => [
                '1' => ['nombre' => 'Lic. Aquiles Bailo', 'email' => 'abailo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Ines Quele', 'email' => 'iquele@uam.edu.ni'],
            ],
            'Derecho Tributario' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
        ],
        'Cuarto Año' => [
            'Derecho Notarial y Registral I' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
            'Derecho Internacional Público' => [
                '1' => ['nombre' => 'Lic. Aitor Tilla', 'email' => 'atilla@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elba Lazo', 'email' => 'elazo@uam.edu.ni'],
            ],
            'Medicina Forense' => [
                '1' => ['nombre' => 'Lic. Aquiles Bailo', 'email' => 'abailo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Ines Quele', 'email' => 'iquele@uam.edu.ni'],
            ],
            'Derecho Ambiental' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
            'Derecho Notarial y Registral II' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
            'Derecho Internacional Privado' => [
                '1' => ['nombre' => 'Lic. Aitor Tilla', 'email' => 'atilla@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elba Lazo', 'email' => 'elazo@uam.edu.ni'],
            ],
            'Criminología' => [
                '1' => ['nombre' => 'Lic. Aquiles Bailo', 'email' => 'abailo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Ines Quele', 'email' => 'iquele@uam.edu.ni'],
            ],
            'Métodos Alternos de Resolución de Conflictos' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
        ],
        'Quinto Año' => [
            'Prácticas Penales y Civiles' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
            'Filosofía del Derecho' => [
                '1' => ['nombre' => 'Lic. Aitor Tilla', 'email' => 'atilla@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elba Lazo', 'email' => 'elazo@uam.edu.ni'],
            ],
            'Técnicas de Litigación Oral' => [
                '1' => ['nombre' => 'Lic. Aquiles Bailo', 'email' => 'abailo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Ines Quele', 'email' => 'iquele@uam.edu.ni'],
            ],
            'Taller de Tesis' => [
                '1' => ['nombre' => 'Lic. Gabriela Solano', 'email' => 'gsolano@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Mateo Rojas', 'email' => 'mrojas@uam.edu.ni'],
            ],
            'Prácticas de Familia, Laboral y Administrativo' => [
                '1' => ['nombre' => 'Lic. Armando Paredes', 'email' => 'aparedes@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elsa Pato', 'email' => 'epato@uam.edu.ni'],
            ],
            'Deontología Jurídica' => [
                '1' => ['nombre' => 'Lic. Aitor Tilla', 'email' => 'atilla@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Elba Lazo', 'email' => 'elazo@uam.edu.ni'],
            ],
        ],
    ],
    'Diplomacia y Relaciones Internacionales' => [
        'Primer Año' => [
            'Introducción a las Relaciones Internacionales' => [
                '1' => ['nombre' => 'Lic. Marco Polo', 'email' => 'mpolo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Juana de Arco', 'email' => 'jarco@uam.edu.ni'],
            ],
            'Historia Universal' => [
                '1' => ['nombre' => 'Lic. Napoleon Bonaparte', 'email' => 'nbonaparte@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Cleopatra Filopator', 'email' => 'cfilopator@uam.edu.ni'],
            ],
            'Teoría de Relaciones Internacionales I' => [
                '1' => ['nombre' => 'Lic. Marco Polo', 'email' => 'mpolo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Juana de Arco', 'email' => 'jarco@uam.edu.ni'],
            ],
            'Derecho Constitucional' => [
                '1' => ['nombre' => 'Lic. Napoleon Bonaparte', 'email' => 'nbonaparte@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Cleopatra Filopator', 'email' => 'cfilopator@uam.edu.ni'],
            ],
            'Introducción al Estudio del Derecho' => [
                '1' => ['nombre' => 'Lic. Marco Polo', 'email' => 'mpolo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Juana de Arco', 'email' => 'jarco@uam.edu.ni'],
            ],
            'Geopolítica' => [
                '1' => ['nombre' => 'Lic. Napoleon Bonaparte', 'email' => 'nbonaparte@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Cleopatra Filopator', 'email' => 'cfilopator@uam.edu.ni'],
            ],
            'Técnicas de Redacción' => [
                '1' => ['nombre' => 'Lic. Marco Polo', 'email' => 'mpolo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Juana de Arco', 'email' => 'jarco@uam.edu.ni'],
            ],
            'Historia de Nicaragua' => [
                '1' => ['nombre' => 'Lic. Napoleon Bonaparte', 'email' => 'nbonaparte@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Cleopatra Filopator', 'email' => 'cfilopator@uam.edu.ni'],
            ],
        ],
        'Segundo Año' => [
            'Teoría de Relaciones Internacionales II' => [
                '1' => ['nombre' => 'Lic. Marco Polo', 'email' => 'mpolo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Juana de Arco', 'email' => 'jarco@uam.edu.ni'],
            ],
            'Economía Internacional' => [
                '1' => ['nombre' => 'Lic. Napoleon Bonaparte', 'email' => 'nbonaparte@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Cleopatra Filopator', 'email' => 'cfilopator@uam.edu.ni'],
            ],
            'Derecho Internacional Público' => [
                '1' => ['nombre' => 'Lic. Marco Polo', 'email' => 'mpolo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Juana de Arco', 'email' => 'jarco@uam.edu.ni'],
            ],
            'Organismos Internacionales I' => [
                '1' => ['nombre' => 'Lic. Napoleon Bonaparte', 'email' => 'nbonaparte@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Cleopatra Filopator', 'email' => 'cfilopator@uam.edu.ni'],
            ],
            'Política Exterior I' => [
                '1' => ['nombre' => 'Lic. Marco Polo', 'email' => 'mpolo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Juana de Arco', 'email' => 'jarco@uam.edu.ni'],
            ],
            'Protocolo y Ceremonial Diplomático' => [
                '1' => ['nombre' => 'Lic. Napoleon Bonaparte', 'email' => 'nbonaparte@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Cleopatra Filopator', 'email' => 'cfilopator@uam.edu.ni'],
            ],
            'Análisis de Política Internacional' => [
                '1' => ['nombre' => 'Lic. Marco Polo', 'email' => 'mpolo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Juana de Arco', 'email' => 'jarco@uam.edu.ni'],
            ],
            'Resolución de Conflictos' => [
                '1' => ['nombre' => 'Lic. Napoleon Bonaparte', 'email' => 'nbonaparte@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Cleopatra Filopator', 'email' => 'cfilopator@uam.edu.ni'],
            ],
        ],
        'Tercer Año' => [
            'Cooperación Internacional' => [
                '1' => ['nombre' => 'Lic. Marco Polo', 'email' => 'mpolo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Juana de Arco', 'email' => 'jarco@uam.edu.ni'],
            ],
            'Derechos Humanos' => [
                '1' => ['nombre' => 'Lic. Napoleon Bonaparte', 'email' => 'nbonaparte@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Cleopatra Filopator', 'email' => 'cfilopator@uam.edu.ni'],
            ],
            'Política Exterior de Nicaragua' => [
                '1' => ['nombre' => 'Lic. Marco Polo', 'email' => 'mpolo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Juana de Arco', 'email' => 'jarco@uam.edu.ni'],
            ],
            'Organismos Internacionales II' => [
                '1' => ['nombre' => 'Lic. Napoleon Bonaparte', 'email' => 'nbonaparte@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Cleopatra Filopator', 'email' => 'cfilopator@uam.edu.ni'],
            ],
            'Integración Regional' => [
                '1' => ['nombre' => 'Lic. Marco Polo', 'email' => 'mpolo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Juana de Arco', 'email' => 'jarco@uam.edu.ni'],
            ],
            'Seguridad Internacional' => [
                '1' => ['nombre' => 'Lic. Napoleon Bonaparte', 'email' => 'nbonaparte@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Cleopatra Filopator', 'email' => 'cfilopator@uam.edu.ni'],
            ],
            'Negociación y Mediación' => [
                '1' => ['nombre' => 'Lic. Marco Polo', 'email' => 'mpolo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Juana de Arco', 'email' => 'jarco@uam.edu.ni'],
            ],
            'Opinión Pública Internacional' => [
                '1' => ['nombre' => 'Lic. Napoleon Bonaparte', 'email' => 'nbonaparte@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Cleopatra Filopator', 'email' => 'cfilopator@uam.edu.ni'],
            ],
        ],
        'Cuarto Año' => [
            'Derecho Diplomático y Consular' => [
                '1' => ['nombre' => 'Lic. Marco Polo', 'email' => 'mpolo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Juana de Arco', 'email' => 'jarco@uam.edu.ni'],
            ],
            'Análisis de Coyuntura' => [
                '1' => ['nombre' => 'Lic. Napoleon Bonaparte', 'email' => 'nbonaparte@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Cleopatra Filopator', 'email' => 'cfilopator@uam.edu.ni'],
            ],
            'Política Exterior de EE.UU.' => [
                '1' => ['nombre' => 'Lic. Marco Polo', 'email' => 'mpolo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Juana de Arco', 'email' => 'jarco@uam.edu.ni'],
            ],
            'Taller de Investigación Diplomática' => [
                '1' => ['nombre' => 'Lic. Napoleon Bonaparte', 'email' => 'nbonaparte@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Cleopatra Filopator', 'email' => 'cfilopator@uam.edu.ni'],
            ],
            'Prácticas Profesionales' => [
                '1' => ['nombre' => 'Lic. Marco Polo', 'email' => 'mpolo@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Juana de Arco', 'email' => 'jarco@uam.edu.ni'],
            ],
            'Seminario de Graduación' => [
                '1' => ['nombre' => 'Lic. Napoleon Bonaparte', 'email' => 'nbonaparte@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Cleopatra Filopator', 'email' => 'cfilopator@uam.edu.ni'],
            ],
        ],
    ],
    'Medicina' => [
        'Primer Año' => [
            'Anatomía Humana I' => [
                '1' => ['nombre' => 'Dr. Alejandro Velez', 'email' => 'avelez@uam.edu.ni'],
                '2' => ['nombre' => 'Dra. Carmen Blanco', 'email' => 'cblanco@uam.edu.ni'],
            ],
            'Histología y Embriología I' => [
                '1' => ['nombre' => 'Dr. Ismael Nieto', 'email' => 'inieto@uam.edu.ni'],
                '2' => ['nombre' => 'Dra. Patricia Montes', 'email' => 'pmontes@uam.edu.ni'],
            ],
            'Bioquímica' => [
                '1' => ['nombre' => 'Dr. Roberto Arias', 'email' => 'rarias@uam.edu.ni'],
                '2' => ['nombre' => 'Dra. Carolina Soto', 'email' => 'csoto@uam.edu.ni'],
            ],
            'Salud Pública I' => [
                '1' => ['nombre' => 'Dr. Esteban Vega', 'email' => 'evega@uam.edu.ni'],
                '2' => ['nombre' => 'Dra. Laura Perez', 'email' => 'lperez@uam.edu.ni'],
            ],
            'Anatomía Humana II' => [
                '1' => ['nombre' => 'Dr. Alejandro Velez', 'email' => 'avelez@uam.edu.ni'],
                '2' => ['nombre' => 'Dra. Carmen Blanco', 'email' => 'cblanco@uam.edu.ni'],
            ],
            'Histología y Embriología II' => [
                '1' => ['nombre' => 'Dr. Ismael Nieto', 'email' => 'inieto@uam.edu.ni'],
                '2' => ['nombre' => 'Dra. Patricia Montes', 'email' => 'pmontes@uam.edu.ni'],
            ],
            'Fisiología Médica I' => [
                '1' => ['nombre' => 'Dr. Ricardo Salas', 'email' => 'rsalas@uam.edu.ni'],
                '2' => ['nombre' => 'Dra. Monica Falla', 'email' => 'mfalla@uam.edu.ni'],
            ],
            'Introducción a la Medicina' => [
                '1' => ['nombre' => 'Dr. Alberto Pinto', 'email' => 'apinto@uam.edu.ni'],
                '2' => ['nombre' => 'Dra. Diana Quiros', 'email' => 'dquiros@uam.edu.ni'],
            ],
        ],
        // ... (resto de la carrera)
    ],
    'Psicología' => [
        'Primer Año' => [
            'Historia de la Psicología' => [
                '1' => ['nombre' => 'Dr. Sigmund Freud', 'email' => 'sfreud@uam.edu.ni'],
                '2' => ['nombre' => 'Dra. Anna O.', 'email' => 'ao@uam.edu.ni'],
            ],
            //...
        ],
    ],
    'Nutrición' => [
        'Primer Año' => [
            'Química General y Orgánica' => [
                '1' => ['nombre' => 'Dr. Marie Curie', 'email' => 'mcurie@uam.edu.ni'],
                '2' => ['nombre' => 'Dr. Louis Pasteur', 'email' => 'lpasteur@uam.edu.ni'],
            ],
            //...
        ],
    ],
    'Odontología' => [
        'Primer Año' => [
            'Anatomía Humana y de Cabeza y Cuello' => [
                '1' => ['nombre' => 'Dr. Pierre Fauchard', 'email' => 'pfauchard@uam.edu.ni'],
                '2' => ['nombre' => 'Dra. Lucy Hobbs Taylor', 'email' => 'lhobbs@uam.edu.ni'],
            ],
            //...
        ],
    ],
    'Arquitectura' => [
        'Primer Año' => [
            'Geometría Descriptiva' => [
                '1' => ['nombre' => 'Ing. Zaha Hadid', 'email' => 'zhadid@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Frank Gehry', 'email' => 'fgehry@uam.edu.ni'],
            ],
            //...
        ],
    ],
    'Ingeniería Civil' => [
        'Primer Año' => [
            'Cálculo I' => [
                '1' => ['nombre' => 'Ing. Isaac Newton', 'email' => 'inewton@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Gottfried Leibniz', 'email' => 'gleibniz@uam.edu.ni'],
            ],
            //...
        ],
    ],
    'Ingeniería Industrial' => [
        'Primer Año' => [
            'Cálculo I' => [
                '1' => ['nombre' => 'Ing. Henry Ford', 'email' => 'hford@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Taiichi Ohno', 'email' => 'tohno@uam.edu.ni'],
            ],
            //...
        ],
    ],
    'Ingeniería en Sistemas de Información' => [
        'Primer Año' => [
            'Cálculo I' => [
                '1' => ['nombre' => 'Ing. Ricardo Palma', 'email' => 'mundogabrielmart@gmail.com'],
                '2' => ['nombre' => 'Ing. Monica Salas', 'email' => 'blueraptoregm24@gmail.com'],
            ],
            'Física I' => [
                '1' => ['nombre' => 'Ing. Alberto Rojas', 'email' => 'arojas@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Diana Quiros', 'email' => 'dquiros@uam.edu.ni'],
            ],
            'Introducción a la Programación' => [
                '1' => ['nombre' => 'Ing. Marcela Solis', 'email' => 'msolis@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Oscar Barquero', 'email' => 'obarquero@uam.edu.ni'],
            ],
            'Comunicación Oral y Escrita' => [
                '1' => ['nombre' => 'Lic. Lucia Cordero', 'email' => 'lcordero@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Fernando Nuñez', 'email' => 'fnunez@uam.edu.ni'],
            ],
            'Cálculo II' => [
                '1' => ['nombre' => 'Ing. Ricardo Palma', 'email' => 'rpalma@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Monica Salas', 'email' => 'msalas@uam.edu.ni'],
            ],
            'Física II' => [
                '1' => ['nombre' => 'Ing. Alberto Rojas', 'email' => 'arojas@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Diana Quiros', 'email' => 'dquiros@uam.edu.ni'],
            ],
            'Programación I' => [
                '1' => ['nombre' => 'Ing. Marcela Solis', 'email' => 'msolis@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Oscar Barquero', 'email' => 'obarquero@uam.edu.ni'],
            ],
            'Álgebra Lineal' => [
                '1' => ['nombre' => 'Ing. Ricardo Palma', 'email' => 'rpalma@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Monica Salas', 'email' => 'msalas@uam.edu.ni'],
            ],
        ],
        'Segundo Año' => [
            'Cálculo III' => [
                '1' => ['nombre' => 'Ing. Ricardo Palma', 'email' => 'rpalma@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Monica Salas', 'email' => 'msalas@uam.edu.ni'],
            ],
            'Programación II' => [
                '1' => ['nombre' => 'Ing. Marcela Solis', 'email' => 'msolis@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Oscar Barquero', 'email' => 'obarquero@uam.edu.ni'],
            ],
            'Contabilidad de Costos' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
            'Estructuras Discretas' => [
                '1' => ['nombre' => 'Ing. Alan Turing', 'email' => 'aturing@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Ada Lovelace', 'email' => 'alovelace@uam.edu.ni'],
            ],
            'Ecuaciones Diferenciales' => [
                '1' => ['nombre' => 'Ing. Ricardo Palma', 'email' => 'rpalma@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Monica Salas', 'email' => 'msalas@uam.edu.ni'],
            ],
            'Programación Orientada a Objetos' => [
                '1' => ['nombre' => 'Ing. Marcela Solis', 'email' => 'msolis@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Oscar Barquero', 'email' => 'obarquero@uam.edu.ni'],
            ],
            'Base de Datos I' => [
                '1' => ['nombre' => 'Ing. Edgar Codd', 'email' => 'ecodd@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Grace Hopper', 'email' => 'ghopper@uam.edu.ni'],
            ],
            'Análisis y Diseño de Sistemas' => [
                '1' => ['nombre' => 'Ing. Grady Booch', 'email' => 'gbooch@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. James Rumbaugh', 'email' => 'jrumbaugh@uam.edu.ni'],
            ],
        ],
        'Tercer Año' => [
            'Probabilidad y Estadística' => [
                '1' => ['nombre' => 'Ing. Ricardo Palma', 'email' => 'rpalma@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Monica Salas', 'email' => 'msalas@uam.edu.ni'],
            ],
            'Base de Datos II' => [
                '1' => ['nombre' => 'Ing. Marcela Solis', 'email' => 'msolis@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Oscar Barquero', 'email' => 'obarquero@uam.edu.ni'],
            ],
            'Sistemas Operativos' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
            'Redes de Computadoras I' => [
                '1' => ['nombre' => 'Ing. Alan Turing', 'email' => 'aturing@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Ada Lovelace', 'email' => 'alovelace@uam.edu.ni'],
            ],
            'Investigación de Operaciones' => [
                '1' => ['nombre' => 'Ing. Ricardo Palma', 'email' => 'rpalma@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Monica Salas', 'email' => 'msalas@uam.edu.ni'],
            ],
            'Ingeniería de Software I' => [
                '1' => ['nombre' => 'Ing. Marcela Solis', 'email' => 'msolis@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Oscar Barquero', 'email' => 'obarquero@uam.edu.ni'],
            ],
            'Redes de Computadoras II' => [
                '1' => ['nombre' => 'Ing. Edgar Codd', 'email' => 'ecodd@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Grace Hopper', 'email' => 'ghopper@uam.edu.ni'],
            ],
            'Programación Web' => [
                '1' => ['nombre' => 'Ing. Grady Booch', 'email' => 'gbooch@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. James Rumbaugh', 'email' => 'jrumbaugh@uam.edu.ni'],
            ],
        ],
        'Cuarto Año' => [
            'Inteligencia Artificial' => [
                '1' => ['nombre' => 'Ing. Ricardo Palma', 'email' => 'rpalma@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Monica Salas', 'email' => 'msalas@uam.edu.ni'],
            ],
            'Seguridad Informática' => [
                '1' => ['nombre' => 'Ing. Marcela Solis', 'email' => 'msolis@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Oscar Barquero', 'email' => 'obarquero@uam.edu.ni'],
            ],
            'Ingeniería de Software II' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
            'Redes Avanzadas' => [
                '1' => ['nombre' => 'Ing. Alan Turing', 'email' => 'aturing@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Ada Lovelace', 'email' => 'alovelace@uam.edu.ni'],
            ],
            'Sistemas de Información Gerencial' => [
                '1' => ['nombre' => 'Ing. Ricardo Palma', 'email' => 'rpalma@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Monica Salas', 'email' => 'msalas@uam.edu.ni'],
            ],
            'Auditoría de Sistemas' => [
                '1' => ['nombre' => 'Ing. Marcela Solis', 'email' => 'msolis@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Oscar Barquero', 'email' => 'obarquero@uam.edu.ni'],
            ],
            'Taller de Graduación I' => [
                '1' => ['nombre' => 'Ing. Edgar Codd', 'email' => 'ecodd@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Grace Hopper', 'email' => 'ghopper@uam.edu.ni'],
            ],
        ],
        'Quinto Año' => [
            'Prácticas Profesionales' => [
                '1' => ['nombre' => 'Ing. Alvaro Rocha', 'email' => 'rpalma@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Monica Salas', 'email' => 'msalas@uam.edu.ni'],
            ],
            'Taller de Graduación II' => [
                '1' => ['nombre' => 'Ing. Marcela Solis', 'email' => 'msolis@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Josue Alonzo Gay', 'email' => 'obarquero@uam.edu.ni'],
            ],
            'Computación en la Nube' => [
                '1' => ['nombre' => 'Lic. Elena Rivera', 'email' => 'erivera@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Marcos Castillo', 'email' => 'mcastillo@uam.edu.ni'],
            ],
            'Desarrollo de Aplicaciones Móviles' => [
                '1' => ['nombre' => 'Ing. Alan Turing', 'email' => 'aturing@uam.edu.ni'],
                '2' => ['nombre' => 'Ing. Ada Lovelace', 'email' => 'alovelace@uam.edu.ni'],
            ],
        ],
    ],
    'Marketing y Publicidad' => [
        'Primer Año' => [
            'Principios de Marketing' => [
                '1' => ['nombre' => 'Lic. Philip Kotler', 'email' => 'pkotler@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Seth Godin', 'email' => 'sgodin@uam.edu.ni'],
            ],
            //...
        ],
    ],
    'Diseño y Comunicación Visual' => [
        'Primer Año' => [
            'Fundamentos del Diseño' => [
                '1' => ['nombre' => 'Lic. Paul Rand', 'email' => 'prand@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Saul Bass', 'email' => 'sbass@uam.edu.ni'],
            ],
            //...
        ],
    ],
    'Comunicación y Relaciones Públicas' => [
        'Primer Año' => [
            'Teoría de la Comunicación' => [
                '1' => ['nombre' => 'Lic. Marshall McLuhan', 'email' => 'mmcluhan@uam.edu.ni'],
                '2' => ['nombre' => 'Lic. Noam Chomsky', 'email' => 'nchomsky@uam.edu.ni'],
            ],
            //...
        ],
    ],
];